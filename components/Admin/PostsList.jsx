...
export default function PostsList({ user }) {
    ...
    const URL = `https://bee-app.herokuapp.com/api/showAll/Posts/byAdmin/${user}`
                ...
                <useEditPost.Provider value={{ selectedEdit, setSelectedEdit, setPostsList, selectedDelete, setSelectedDelete }}>
                    <>
                        ...
                        {!error && !loader && postsList?.result?.map(post => {
                            ...
                            return <CardAdmin img={post.pub_media} title={post.pub_titulo} body={post.pub_descripcion} date={post.pub_fecha} hour={post.pub_horainicio} place={post.pub_locacion} key={post.pub_id} id={post.pub_id} user={user} status={true} />
                        })}
                    </>
