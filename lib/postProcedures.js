...
const editUserName = async (name, id) => {
    const { updateUser: { nombre } } = MyQuerys

    const query = format(nombre, [
        name,
        id
    ])
...
const editUserEmail = async (email, id) => {
    const { updateUser: { correo } } = MyQuerys

    const query = format(correo, [
        email,
        id
    ])
...
const editUserRfc = async (myRfc, id) => {
    const { updateUser: {rfc} } = MyQuerys

    const query = format(rfc, [
        myRfc,
        id
    ])
...
const editUserCedula = async (myCedula, id) => {
    const { updateUser: {cedula} } = MyQuerys

    const query = format(cedula, [
        myCedula,
        id
    ])
...
export { newPost, newPropost, deletePost, updatePost, updatePropost, newComment, addLike, fetchIFLikeU, deleteLike, fetchPostNLike, deleteComments, deleteLikes ,editUserName, editUserRfc, editUserEmail, editUserCedula }